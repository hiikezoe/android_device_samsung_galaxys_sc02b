...
LOCAL_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), os.pardir, "aries-common")
...
def FullOTA_Assertions(info):
  ...
  info.output_zip.write(os.path.join(TARGET_DIR, "modem.bin.md5sum"), "modem.bin.md5sum")
  ...
  info.script.AppendExtra(
        ('package_extract_file("modem.bin", "/tmp/modem.bin");\n'
         'set_perm(0, 0, 0444, "/tmp/modem.bin");'))
  info.script.AppendExtra(
        ('package_extract_file("modem.bin.md5sum", "/tmp/modem.bin.md5sum");\n'
         'set_perm(0, 0, 0444, "/tmp/modem.bin.md5sum");'))
  info.script.AppendExtra(
